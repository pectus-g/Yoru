    ...
    public partial class XFurStudioInstance : MonoBehaviour {
        ...
        protected void RenderFur() {
            ...
            if ( IsSkinnedMesh ) {
                ...
                if ( IsSkinnedMesh ) {
                    if ( (CurrentFurRenderer.renderer as SkinnedMeshRenderer ).rootBone ) {
                        tPos = ( CurrentFurRenderer.renderer as SkinnedMeshRenderer ).rootBone.position;
                    }
                }
        ...
        protected void LateUpdate() {
            ...
            if ( IsSkinnedMesh ) {
                ...
                if ( _settings.useVertexBuffer ) {
                    ...
                    Vector3 position = CurrentFurRenderer.renderer.transform.position;

                    if ( IsSkinnedMesh ) {
                        if ( (CurrentFurRenderer.renderer as SkinnedMeshRenderer ).rootBone ) {
                            position = ( CurrentFurRenderer.renderer as SkinnedMeshRenderer ).rootBone.position;
                        }
                    }

                    matricesJob.originalMatrix = Matrix4x4.TRS( position, ( CurrentFurRenderer.renderer as SkinnedMeshRenderer ).rootBone? ( CurrentFurRenderer.renderer as SkinnedMeshRenderer ).rootBone.rotation: CurrentFurRenderer.renderer.transform.rotation, CurrentFurRenderer.renderer.transform.localScale );
                }
                else {
