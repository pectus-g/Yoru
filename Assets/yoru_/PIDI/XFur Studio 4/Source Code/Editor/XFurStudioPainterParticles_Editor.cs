...
namespace XFurStudio.Editor {
    ...
    [CustomEditor( typeof( XFurStudioPainterParticles ) )]
    public class XFurStudioPainterParticles_Editor : Editor {


        public GUISkin pidiSkin2;
        public Texture2D xfurStudioLogo;
        public XFurStudioPainterParticles painterParticles;
        ...
        private void OnEnable() {
            ...
            painterParticles = (XFurStudioPainterParticles)target;
        ...
        public override void OnInspectorGUI() {
            ...
            Undo.RecordObject( painterParticles, "XFurInstance_" + GetInstanceID() );


            GUILayout.Space( 16 );

            painterParticles.PaintDataMode = (XFurStudioAPI.PaintDataMode)EditorGUILayout.EnumPopup( new GUIContent( "XFur Paint Mode" ), painterParticles.PaintDataMode, EditorGUIUtility.isProSkin ? pidiSkin2.button : EditorGUIUtility.GetBuiltinSkin( EditorSkin.Inspector ).button );

            GUILayout.Space( 12 );

            //painterParticles.painterParticlesShape = (XFurStudiopainterParticles.PainterShape)StandardEnumField( new GUIContent( "Painter Shape", "The shape used by this object to paint XFur Instances" ), painterParticles.painterParticlesShape );

            //painterParticles.painterParticlesShape = XFurStudiopainterParticles.PainterShape.Spherical;

            painterParticles.SyncParticleData = EditorGUILayout.Toggle( "Sync to ParticleSystem", painterParticles.SyncParticleData );

            if ( !painterParticles.SyncParticleData ) {
                painterParticles.Radius = EditorGUILayout.FloatField( new GUIContent( "Radius" ), painterParticles.Radius );
            }
            //else {
            //   painterParticles.BoxSize = EditorGUILayout.Vector3Field( new GUIContent( "Size" ), painterParticles.BoxSize );
            // }

            GUILayout.Space( 12 );

            if ( painterParticles.PaintDataMode == XFurStudioAPI.PaintDataMode.SnowFX || painterParticles.PaintDataMode == XFurStudioAPI.PaintDataMode.BloodFX || painterParticles.PaintDataMode == XFurStudioAPI.PaintDataMode.WaterFX ) {

                if (!painterParticles.SyncParticleData)
                    painterParticles.BrushOpacity = EditorGUILayout.Slider( new GUIContent( "Effect Intensity" ), painterParticles.BrushOpacity, 0.0f, 10.0f );
                
                painterParticles.InvertBrush = EnableDisableToggle( new GUIContent( "Invert Value", "Inverts the value to paint. For the fur mask, shaves fur instead of growing it. For Fur Length, it makes the fur shorter instead of longer, for Fur Occlusion removes it instead of adding it, etc." ), painterParticles.InvertBrush, true );

            }
            else {

                if ( painterParticles.PaintDataMode == XFurStudioAPI.PaintDataMode.FurColor ) {
                    if ( !painterParticles.SyncParticleData )
                        painterParticles.BrushColor = EditorGUILayout.ColorField( new GUIContent( "Brush Color" ), painterParticles.BrushColor );
                }

                if ( painterParticles.PaintDataMode != XFurStudioAPI.PaintDataMode.FurMask ) {
                    if ( !painterParticles.SyncParticleData ) {
                        painterParticles.BrushOpacity = EditorGUILayout.Slider( new GUIContent( "Effect Intensity" ), painterParticles.BrushOpacity, 0, 1 );
                        painterParticles.BrushHardness = EditorGUILayout.Slider( new GUIContent( "Brush Hardness" ), painterParticles.BrushHardness, 0, 1 );
                    }
                }

                if ( painterParticles.PaintDataMode == XFurStudioAPI.PaintDataMode.FurMask || painterParticles.PaintDataMode == XFurStudioAPI.PaintDataMode.FurLength || painterParticles.PaintDataMode == XFurStudioAPI.PaintDataMode.FurThickness || painterParticles.PaintDataMode == XFurStudioAPI.PaintDataMode.FurOcclusion ) {
                    painterParticles.InvertBrush = EnableDisableToggle( new GUIContent( "Invert Value", "Inverts the value to paint. For the fur mask, shaves fur instead of growing it. For Fur Length, it makes the fur shorter instead of longer, for Fur Occlusion removes it instead of adding it, etc." ), painterParticles.InvertBrush, true ); 
                }
        ...
        public void AssetLogoAndVersion() {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            GUILayout.Label( "4.1.0", pidiSkin2.customStyles[2] );
            GUILayout.Space( 6 );
            GUILayout.EndHorizontal();
            GUILayout.EndVertical();
        }
