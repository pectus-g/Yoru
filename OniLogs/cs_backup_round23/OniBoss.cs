...
public class OniBoss : MonoBehaviour
{
    ...
    [Tooltip("Optional prefab spawned at the swing's strike moment. LEAVE EMPTY to use the built-in procedural crescent instead (round 22) — no asset, no FBX, nothing to wire. Fill it only if you would rather use an authored effect from one of the VFX packs.")]
    [SerializeField] private GameObject swingWaveVFX;
    [Tooltip("ROUND 22. Colour of the procedural crescent. Warm orange reads as displaced air against the cave's cold light; try a pale blue-white for a cleaner, colder swing.")]
    [SerializeField] private Color swingWaveColor = new Color(1f, 0.72f, 0.42f);
    [Tooltip("ROUND 22. Seconds the crescent takes to sweep out and fade. Short reads as a snap of air; long reads as a slow pressure wave.")]
    [SerializeField] private float swingWaveVisualDuration = 0.35f;
    ...
    private void UpdateSwingWave()
    {
        ...
        {
            // ROUND 22: no prefab needed. Spawned at HIS position (not offset forward) and given
            // the same radius and half-angle the damage check uses, so the crescent draws exactly
            // the area that can hurt her — the telegraph and the hitbox are the same shape.
            ProceduralImpactFX.SwingArc(transform.position + Vector3.up * swingWaveVFXHeight,
                                        transform.forward,
                                        swingWaveRadius,
                                        swingWaveHalfAngle,
                                        swingWaveVisualDuration,
                                        swingWaveColor);
        }
