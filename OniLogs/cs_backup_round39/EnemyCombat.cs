   ...
   private void DealDamageToPlayer()
    {
        ...
        if (dist > currentAttack.range)
        {
            ...
            StrikeResolved?.Invoke(false);   // ROUND 38: the miss is a result too — the Oni's ground wave spawns off it
            return;
        }

        PlayerHealth playerHealth = player.GetComponent<PlayerHealth>();
        if (playerHealth == null) { StrikeResolved?.Invoke(false); return; }
