...
public class OniBoss : MonoBehaviour
{
    ...
    private float chargeTrailLastLogTime = -10f;   // ROUND 46: one trail-spawn proof line per rush
                ...
                if (!arrived && !timedOut)
                {
                    ...
                    if (chargeGroundTrailVFX != null)
                    {
                        ...
                        if (chargeWaveTimer <= 0f)
                        {
                            ...
                            // ROUND 46: force the particles to PLAY — a prefab authored with
                            // Play On Awake off spawns silently invisible, which is exactly
                            // "no fire at all" while everything else works.
                            foreach (var ps in puff.GetComponentsInChildren<ParticleSystem>(true))
                                if (!ps.isPlaying) ps.Play(true);

                            Destroy(puff, Mathf.Max(0.2f, chargeTrailLifetime));

                            // Once per rush: proof in the log that the trail is really spawning.
                            if (Time.time - chargeTrailLastLogTime > 3f)
                            {
                                chargeTrailLastLogTime = Time.time;
                                DebugLog($"charge fire trail: spawning '{chargeGroundTrailVFX.name}' every {chargeWaveInterval:F2}s at his feet, each living {chargeTrailLifetime:F1}s.");
                            }
                        }
                    }
